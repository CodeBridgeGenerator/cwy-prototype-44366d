...
import jobApplicationReviewFakerFactory from "./jobApplicationReviewFakerFactory";
...
const JobApplicationReviewFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [jobAppIDIds, setjobAppIDIds] = useState();
const [adminIds, setadminIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("jobApplications")
                        .find({ query: { $select: ["_id","jobApplicationID"] } })
                        .then((data) => {
                        setjobAppIDIds(data.data);
                        });
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = jobApplicationReviewFakerFactory(props.user,fakerCount,jobAppIDIds,adminIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobApplicationReview").create(elem));
...
export default connect(mapState, mapDispatch)(JobApplicationReviewFakerDialogComponent);
