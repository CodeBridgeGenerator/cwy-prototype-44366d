...
const JobApplicationReviewDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.jobAppID?.jobApplicationID}</p>
...
const tickTemplate4 = (rowData, { rowIndex }) => <i className={`pi ${rowData.approvalStatus?"pi-check": "pi-times"}`}  ></i>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "jobApplicationReview"});
...
<Column field="jobAppID" header="Job Application ID" body={dropdownTemplate0} filter={selectedFilterFields.includes("jobAppID")} hidden={selectedHideFields?.includes("jobAppID")}  style={{ minWidth: "8rem" }} />
...
<Column field="approvalStatus" header="Approval Status" body={tickTemplate4} filter={selectedFilterFields.includes("approvalStatus")} hidden={selectedHideFields?.includes("approvalStatus")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload JobApplicationReview Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search JobApplicationReview" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default JobApplicationReviewDataTable;