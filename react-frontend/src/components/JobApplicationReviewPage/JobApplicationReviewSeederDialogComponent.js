...
import seederData from "./jobApplicationReview.seeder.json";
...
const JobApplicationReviewSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("jobApplicationReview").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(JobApplicationReviewSeederDialogComponent);
