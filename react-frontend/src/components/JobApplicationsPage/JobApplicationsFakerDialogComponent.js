...
import jobApplicationsFakerFactory from "./jobApplicationsFakerFactory";
...
client.service("jobs")
                        .find({ query: { $select: ["_id","jobTitle"] } })
                        .then((data) => {
                        setjobNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = jobApplicationsFakerFactory(props.user,fakerCount,NameIds,jobNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobApplications").create(elem));
...
export default connect(mapState, mapDispatch)(JobApplicationsFakerDialogComponent);
