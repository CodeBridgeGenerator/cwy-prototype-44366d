...
import seederData from "./memberProfile.seeder.json";
...
const MemberProfileSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("memberProfile").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(MemberProfileSeederDialogComponent);
