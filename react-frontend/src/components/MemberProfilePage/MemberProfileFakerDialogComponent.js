...
import memberProfileFakerFactory from "./memberProfileFakerFactory";
...
const MemberProfileFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = memberProfileFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memberProfile").create(elem));
...
export default connect(mapState, mapDispatch)(MemberProfileFakerDialogComponent);
