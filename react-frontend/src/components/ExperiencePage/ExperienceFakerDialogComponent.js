...
import experienceFakerFactory from "./experienceFakerFactory";
...
const ExperienceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = experienceFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("experience").create(elem));
...
export default connect(mapState, mapDispatch)(ExperienceFakerDialogComponent);
