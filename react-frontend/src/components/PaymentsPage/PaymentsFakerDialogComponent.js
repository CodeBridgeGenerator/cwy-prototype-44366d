...
import paymentsFakerFactory from "./paymentsFakerFactory";
...
const PaymentsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [payeeNameIds, setpayeeNameIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        ...
                        .then((data) => {
                        setpayeeNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = paymentsFakerFactory(props.user,fakerCount,payeeNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("payments").create(elem));
...
export default connect(mapState, mapDispatch)(PaymentsFakerDialogComponent);
