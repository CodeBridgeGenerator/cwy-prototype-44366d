...
import educationFakerFactory from "./educationFakerFactory";
...
const EducationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = educationFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("education").create(elem));
...
export default connect(mapState, mapDispatch)(EducationFakerDialogComponent);
