...
import scholarshipApplicationsFakerFactory from "./scholarshipApplicationsFakerFactory";
...
const ScholarshipApplicationsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [nameIds, setnameIds] = useState();
const [scholarshipNameIds, setscholarshipNameIds] = useState();
...
client.service("scholarships")
                        .find({ query: { $select: ["_id","scholarshipName"] } })
                        .then((data) => {
                        setscholarshipNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = scholarshipApplicationsFakerFactory(props.user,fakerCount,nameIds,scholarshipNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("scholarshipApplications").create(elem));
...
export default connect(mapState, mapDispatch)(ScholarshipApplicationsFakerDialogComponent);
