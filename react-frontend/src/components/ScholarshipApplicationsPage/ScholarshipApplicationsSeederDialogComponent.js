...
import seederData from "./scholarshipApplications.seeder.json";
...
const ScholarshipApplicationsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("scholarshipApplications").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ScholarshipApplicationsSeederDialogComponent);
