...
import InstitutionsCreateDialogComponent from "./InstitutionsCreateDialogComponent";
import InstitutionsFakerDialogComponent from "./InstitutionsFakerDialogComponent";
import InstitutionsSeederDialogComponent from "./InstitutionsSeederDialogComponent";
...
const InstitutionsPage = (props) => {
    ...
    const filename = "institutions.csv";
    ...
    const service = _.find(config.services, { serviceName : "institutions"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("institutions").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Institutions", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("institutions").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/institutions/${data._id}`);
    };
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Institutions </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="institutions-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="institutions-datatable">
                    <InstitutionsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <InstitutionsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <InstitutionsCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <InstitutionsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <InstitutionsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(InstitutionsPage);
