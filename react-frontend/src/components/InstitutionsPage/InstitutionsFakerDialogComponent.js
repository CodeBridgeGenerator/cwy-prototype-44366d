...
import institutionsFakerFactory from "./institutionsFakerFactory";
...
const InstitutionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = institutionsFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("institutions").create(elem));
...
export default connect(mapState, mapDispatch)(InstitutionsFakerDialogComponent);
