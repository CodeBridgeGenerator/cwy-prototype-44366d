...
const InstitutionsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.InstitutionName}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.Location}</p>
const dropdownTemplate2 = (rowData, { rowIndex }) => <p >{rowData.name?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "institutions"});
...
<Column field="InstitutionName" header="Institution Name" body={pTemplate0} filter={selectedFilterFields.includes("InstitutionName")} hidden={selectedHideFields?.includes("InstitutionName")}  sortable style={{ minWidth: "8rem" }} />
<Column field="Location" header="Location" body={pTemplate1} filter={selectedFilterFields.includes("Location")} hidden={selectedHideFields?.includes("Location")}  sortable style={{ minWidth: "8rem" }} />
<Column field="name" header="Name" body={dropdownTemplate2} filter={selectedFilterFields.includes("name")} hidden={selectedHideFields?.includes("name")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Institutions Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Institutions" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
