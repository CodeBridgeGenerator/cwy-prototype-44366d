...
import memberInstitutionsFakerFactory from "./memberInstitutionsFakerFactory";
...
const MemberInstitutionsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [nameIds, setnameIds] = useState();
const [institutionNameIds, setinstitutionNameIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("memberProfile")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setnameIds(data.data);
                        });
client.service("institutions")
                        .find({ query: { $select: ["_id","InstitutionName"] } })
                        .then((data) => {
                        setinstitutionNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = memberInstitutionsFakerFactory(props.user,fakerCount,nameIds,institutionNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memberInstitutions").create(elem));
...
export default connect(mapState, mapDispatch)(MemberInstitutionsFakerDialogComponent);
