...
const MemberInstitutionsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.institutionName?.InstitutionName}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "memberInstitutions"});
...
<Column field="institutionName" header="Institution Name" body={dropdownTemplate1} filter={selectedFilterFields.includes("institutionName")} hidden={selectedHideFields?.includes("institutionName")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload MemberInstitutions Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search MemberInstitutions" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default MemberInstitutionsDataTable;