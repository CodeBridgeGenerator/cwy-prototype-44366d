...
import scholarshipsFakerFactory from "./scholarshipsFakerFactory";
...
const ScholarshipsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = scholarshipsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("scholarships").create(elem));
...
export default connect(mapState, mapDispatch)(ScholarshipsFakerDialogComponent);
