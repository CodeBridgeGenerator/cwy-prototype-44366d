...
import jobsFakerFactory from "./jobsFakerFactory";
...
const JobsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = jobsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobs").create(elem));
...
export default connect(mapState, mapDispatch)(JobsFakerDialogComponent);
