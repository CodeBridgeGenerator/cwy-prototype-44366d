...
import seederData from "./scholarshipApplicationReview.seeder.json";
...
const ScholarshipApplicationReviewSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("scholarshipApplicationReview").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ScholarshipApplicationReviewSeederDialogComponent);
