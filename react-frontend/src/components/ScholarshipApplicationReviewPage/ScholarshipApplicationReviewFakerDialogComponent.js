...
import scholarshipApplicationReviewFakerFactory from "./scholarshipApplicationReviewFakerFactory";
...
const ScholarshipApplicationReviewFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [scholarshipAppIDIds, setscholarshipAppIDIds] = useState();
const [adminIds, setadminIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("scholarshipApplications")
                        .find({ query: { $select: ["_id","scholarshipApplicationID"] } })
                        .then((data) => {
                        setscholarshipAppIDIds(data.data);
                        });
client.service("users")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setadminIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = scholarshipApplicationReviewFakerFactory(props.user,fakerCount,scholarshipAppIDIds,adminIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("scholarshipApplicationReview").create(elem));
...
export default connect(mapState, mapDispatch)(ScholarshipApplicationReviewFakerDialogComponent);
