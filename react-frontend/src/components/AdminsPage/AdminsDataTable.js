...
const AdminsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.AdminID}</p>
...
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.password}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "admins"});
...
<Column field="AdminID" header="AdminID" body={pTemplate0} filter={selectedFilterFields.includes("AdminID")} hidden={selectedHideFields?.includes("AdminID")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="password" header="Password" body={pTemplate3} filter={selectedFilterFields.includes("password")} hidden={selectedHideFields?.includes("password")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Admins Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Admins" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
