...
import AdminsEditDialogComponent from "./AdminsEditDialogComponent";
import AdminsCreateDialogComponent from "./AdminsCreateDialogComponent";
import AdminsFakerDialogComponent from "./AdminsFakerDialogComponent";
import AdminsSeederDialogComponent from "./AdminsSeederDialogComponent";
...
const AdminsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                console.log({ error });
                setLoading(false);
                props.alert({ title: "Admins", type: "error", message: error.message || "Failed get Admins" });
            });

    const service = _.find(config.services, { serviceName : "admins"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("admins").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Admins", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("admins").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Admins </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="admins-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="admins-datatable">
                    <AdminsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <AdminsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AdminsCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <AdminsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AdminsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AdminsPage);
