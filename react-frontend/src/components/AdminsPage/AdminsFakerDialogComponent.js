...
import adminsFakerFactory from "./adminsFakerFactory";
...
const AdminsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = adminsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("admins").create(elem));
...
export default connect(mapState, mapDispatch)(AdminsFakerDialogComponent);
