...
const AdminsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            AdminID: _entity?.AdminID,username: _entity?.username,email: _entity?.email,password: _entity?.password,
        ...
        const result = await client.service("admins").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Admins created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Admins" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Admins" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="admins-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="password" className="w-full mb-3 p-inputtext-sm" value={_entity?.password} onChange={(e) => setValByKey("password", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(AdminsCreateDialogComponent);
